...
export function CallDetailsDialog({ callId, open, onOpenChange, children }) {
  ...
  const parseTranscript = () => {
    ...
    lines.forEach(line => {
      ...
      // Try to extract speaker ID from line (format: "SPEAKER_00: text")
      const match = trimmed.match(/^(SPEAKER_\d+):\s*(.+)$/);
      ...
      if (match) {
        ...
        // If no speaker ID found, add as is
        parsed.push({
          speaker: 'unknown',
          role: 'unknown',
          text: trimmed
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 p-4 bg-muted rounded-lg">
              ...
              <div>
                ...
                <p className="font-medium">{callData.duration ? `${callData.duration}s` : 'N/A'}</p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground">Score</p>
                <p className="font-medium">{callData.score ? `${callData.score}/100` : 'N/A'}</p>
              </div>
              <div>
                    ...
                    {transcriptLines.map((line, index) => (
                      <div 
                        ...
                        className={`flex gap-3 ${
                          line.role === 'agent' ? 'bg-blue-50' : 
                          line.role === 'caller' ? 'bg-green-50' : 
                          'bg-gray-50'
                        } p-3 rounded-lg`}
                      >
